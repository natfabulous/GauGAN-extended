...
program
  ...
  .option('-w, --working [string]', 'working directory where intermediary info is saved')
...
;(async () => {
  if (!options.skipToMux){
    ...
    let files = [], e
    while ((e = dir.readSync()) !== null) { files.push(imagedir+'\\'+e.name)}
    dir.closeSync()
    ...
    await framesToVideo(options.working,options.targetVideoFile,{sourceVideo:options.video})
  }
})()